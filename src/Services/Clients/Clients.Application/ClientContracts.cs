...
public sealed record CreateClientRequest(string FirstName, string LastName, string Gender, int Age, string Identification, string Address, string Phone, string ClientId, string Password);
...
public sealed record ClientResponse(Guid Id, string ClientId, string FirstName, string LastName, string Gender, int Age, string Identification, string Address, string Phone, bool IsActive, DateTimeOffset CreatedAt);
...
public interface IClientRepository
{
    ...
    Task<bool> ExistsByIdentificationAsync(string identification, Guid? excludingId, CancellationToken cancellationToken);
    Task<bool> ExistsByClientIdAsync(string clientId, Guid? excludingId, CancellationToken cancellationToken);
...
/// <summary>Publica cambios del cliente como eventos de integración.</summary>
public interface IClientEventPublisher
{
    Task PublishAsync(Client client, CancellationToken cancellationToken);
}

internal static class ClientMapping
{
    public static ClientResponse ToResponse(this Client client) => new(client.Id, client.ClientId, client.FirstName, client.LastName, client.Gender, client.Age, client.Identification, client.Address, client.Phone, client.IsActive, client.CreatedAt);
}