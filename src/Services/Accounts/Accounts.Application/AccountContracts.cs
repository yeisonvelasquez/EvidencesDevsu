...
public sealed record CreateTransactionRequest(decimal Amount, TransactionType Type, string? IdempotencyKey);

/// <summary>Representación pública de una cuenta.</summary>
public sealed record AccountResponse(Guid Id, string AccountNumber, string AccountType, decimal Balance, bool IsActive, Guid ClientId, string? ClientName);

/// <summary>Cuenta junto con el nombre del cliente proyectado localmente.</summary>
public sealed record AccountReadModel(Account Account, string? ClientName);
...
{
    public static AccountResponse ToResponse(this AccountReadModel x) => new(x.Account.Id, x.Account.AccountNumber, x.Account.AccountType, x.Account.Balance, x.Account.IsActive, x.Account.ClientId, x.ClientName);
